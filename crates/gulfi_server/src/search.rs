...
pub struct SearchParams {
    ...
    pub document: String,
    pub strategy: gulfi_types::SearchStrategy,
    pub peso_fts: f32,
...
#[instrument(name = "searching", skip(state, client, params), fields(source = tracing::field::Empty))]
pub async fn search_stream(
    ...
    let s = async_stream::stream! {
        let search_result = match prepare_search(&state, &params).await {
            Ok(res) => res,
            Err(e) => {
...
async fn prepare_search(
    ...
    let query = Query::parse(&format!("query: {}", params.search_str)).map_err(HttpError::from)?;
...
async fn stream_results(
    ...
    tokio::task::spawn_blocking(move || -> eyre::Result<()> {
        let (sql, binding_values) = build_query(&search, query_emb)?;
...
fn build_query(
    ...
    let result = match search.strategy {
        SearchStrategy::Fts => build_fts_query(search),
        SearchStrategy::Semantic => build_semantic_query(search, query_emb)?,
        SearchStrategy::ReciprocalRankFusion => build_rrf_query(search, query_emb)?,
    };
    Ok(result)
}
...
struct StreamSearch {
    document: Document,
    query: Query,
    strategy: gulfi_types::SearchStrategy,
    k_neighbors: u64,
    weight_fts: f32,
    weight_vec: f32,
}
