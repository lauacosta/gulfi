var listenerResetHeldData=(()=>{const a=document.getElementById(`resetDownloadBtn`);a&&a.addEventListener(`click`,a=>{a.preventDefault();heldData=[];const b=calcularPeso(heldData);itemsCount.textContent=heldData.length;dataWeight.textContent=`${b} KB`;downloadBtn.disabled=!0})});var calcularPeso=(a=>{const b=JSON.stringify(a);const c=new Blob([b]).size;return (c/1024).toFixed(2)});var hideElements=(()=>{const a=document.querySelectorAll(`.ocultable`);const b=document.getElementById(`strategy`);const c=document.querySelector(`.balance-slider`);if(!a||!b||!c)return undefined;if(b.value===`ReciprocalRankFusion`){c.style.display=`block`;for(const b of a)b.style.display=`block`}else{c.style.display=`none`;if(b.value===`Fts`)for(const b of a)b.style.display=`none`;else for(const b of a)b.style.display=`block`};b.addEventListener(`change`,()=>{if(b.value===`ReciprocalRankFusion`){c.style.display=`block`;for(const b of a)b.style.display=`block`}else{c.style.display=`none`;if(b.value===`Fts`)for(const b of a)b.style.display=`none`;else for(const b of a)b.style.display=`block`}})});var descargarCSVFavoritos=((a,b)=>{if(!a)return undefined;try{const c=JSON.parse(a);if(!c||c.length===0)return undefined;const d=[];for(const a of c)d.push(`"${a.replace(/"/g,`""`)}"`);const e=d.join(`
`);const f=new Blob([e],{type:`text/csv`});const g=document.createElement(`a`);g.href=URL.createObjectURL(f);g.download=`${b}.csv`;g.click();URL.revokeObjectURL(g.href)}catch(b){console.error(b);console.log(a)}});var initHistorial=(()=>{const a=document.querySelectorAll(`.list-item`);const b=document.getElementById(`search-input`);for(const c of a)c.addEventListener(`click`,a=>{a.preventDefault();const d=c.textContent||``;b.value=d.trim()})});var guardarResultados=(()=>{const a=document.getElementById(`table-content`);const b=document.getElementById(`itemsCount`);const c=document.getElementById(`dataWeight`);const d=document.getElementById(`downloadBtn`);const e=a.querySelectorAll(`thead th`);e.forEach((e,f)=>{e.addEventListener(`mousedown`,()=>{if(!isHolding){isHolding=!0;heldHeaders.push(e.innerText);heldData.push(...[...a.querySelectorAll(`tbody tr`)].map(a=>{const b=a.children[f];return b?b.innerText.trim().replace(/\n/g,``):``}));const g=calcularPeso(heldData);b.textContent=heldData.length;c.textContent=`${g} KB`;d.disabled=!1;const h=document.getElementById(`search-input`);h!==null&&h.value.trim()&&heldSearches.push(h.value)}});e.addEventListener(`mouseup`,()=>{isHolding=!1})});d&&d.addEventListener(`click`,async(a)=>{a.preventDefault();descargarCSVGlobal()})});var initPagination=(()=>{var g=(a=>{const c=a*b;const f=c+ b;d.forEach((a,b)=>{a.style.display=b>=c&&b<f?``:`none`});i(a,e)});var i=((a,b)=>{const c=document.querySelector(`.page-info`);c.textContent=`Página ${a+ 1} de ${b}`});var h=((a,b,d)=>{const f=document.createElement(`button`);f.textContent=`<<`;f.addEventListener(`click`,a=>{a.preventDefault();c=0;d(c)});const g=document.createElement(`button`);g.textContent=`<`;g.addEventListener(`click`,a=>{a.preventDefault();c>0&&d(--c)});const h=document.createElement(`span`);h.classList.add(`page-info`);const j=document.createElement(`button`);j.textContent=`>`;j.addEventListener(`click`,a=>{a.preventDefault();c<e- 1&&d(++c)});const k=document.createElement(`button`);k.textContent=`>>`;k.addEventListener(`click`,a=>{a.preventDefault();c=e- 1;d(c)});a.append(f,g,h,j,k);i(c,b)});const a=document.querySelector(`.modern-table`);if(!a)return undefined;const b=10;let c=0;const d=Array.from(a.getElementsByTagName(`tr`)).slice(1);const e=Math.ceil(d.length/b);const f=document.querySelector(`.pagination`);h(f,e,g);g(c)});var updateHistorial=(async()=>{const a=document.getElementById(`historial`);if(!a)return undefined;try{const b=await fetch(`/historial`);if(!b.ok)return a.innerHTML=`<li>Ha ocurrido un error.</li>`,undefined;const c=await b.json();a.innerHTML=``;if(c.length===0)return a.innerHTML=`<li>No se encuentran elementos.</li>`,undefined;for(const b of c){const c=document.createElement(`li`);c.textContent=b.query;c.classList.add(`list-item`);const d=document.createElement(`button`);d.innerHTML=`
